...
const contactInfo = [
  { icon: Mail, label: "Email", value: "muhammadulhasnain@gmail.com", href: "mailto:muhammadulhasnain@gmail.com" },
  { icon: Phone, label: "Phone", value: "(+92) 311 056 9228", href: "tel:+923110569228" },
...
const socialLinks = [
  ...
  { icon: MessageCircle, href: "https://t.me/JosephCryll", label: "Telegram" },
];
...
const ContactSection = () => {
  ...
  return (
    <section id="contact" className="section-padding relative" ref={ref}>
      <div data-parallax-bg="0.2" className="absolute bottom-0 left-0 w-96 h-96 bg-primary/10 rounded-full blur-3xl pointer-events-none" />
      <div data-parallax-bg="0.15" className="absolute top-1/4 right-0 w-64 h-64 bg-accent/10 rounded-full blur-3xl pointer-events-none" />
              ...
              {contactInfo.map((item, index) => (
                ...
                >
                  ...
                  <div>
                    ...
                    {item.href ? (
                      <a href={item.href} className="text-foreground hover:text-primary transition-colors font-medium">{item.value}</a>
                    ) : (
          ...
          >
            <div className="space-y-6">
              <div>
                ...
                <input type="text" id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required className="w-full px-4 py-3 bg-secondary/50 border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary transition-colors" placeholder="Your name" />
              </div>
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-foreground mb-2">Email</label>
                <input type="email" id="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required className="w-full px-4 py-3 bg-secondary/50 border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary transition-colors" placeholder="your@email.com" />
              </div>
              <div>
                <label htmlFor="message" className="block text-sm font-medium text-foreground mb-2">Message</label>
                <textarea id="message" value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} required rows={5} className="w-full px-4 py-3 bg-secondary/50 border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary transition-colors resize-none" placeholder="Tell me about your project..." />
              </div>
              <MagneticButton
                as="button"
